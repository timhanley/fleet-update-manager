...
```
http://localhost:8484         ← status dashboard
http://localhost:8484/admin   ← fleet management
```
...
| `reboot_if_required` | `true` | Reboot after updates if `/var/run/reboot-required` exists |
| `reboot_delay_minutes` | `1` | Minutes to wait before issuing reboot |
...
| ↻ Reboot needed | Updates applied but device needs a reboot |
...
If your devices use `.local` mDNS hostnames, the container needs to be on the same network segment as the devices, and the host's mDNS resolver may not be forwarded into Docker by default. Options:
- Use `network_mode: host` in `docker-compose.yml` (Linux only)
- Use static IP addresses instead of `.local` names
- Add device entries to `/etc/hosts` inside the container via `extra_hosts` in `docker-compose.yml`
